...
deadlineTasks=$(echo "$deadlineTasks" | grep -E "^\s*(Due|Overdue)" | sed -E 's/[0-9]{1,2}:[0-9]{2}\.+\s+//')
...
while IFS= read -r line; do
	...
	deadline=$(echo "$line" | sed -E 's/^\s*(Due today|Due in [0-9]+ days|Overdue [0-9]+ days)?.*/\1/')
    status=$(echo "$line" | sed -E 's/^\s*(Due today|Due in [0-9]+ days|Overdue [0-9]+ days)?\s*([A-Z-]+).*/\2/')
	priority=$(echo "$line" | sed -E 's/^\s*(Due today|Due in [0-9]+ days|Overdue [0-9]+ days)?\s*[A-Z-]+[ \t]+\[#([A-Z])\].*/\2/')
	title=$(echo "$line" | sed -E 's/^\s*(Due today|Due in [0-9]+ days|Overdue [0-9]+ days)?\s*[A-Z-]+[ \t]+\[#[A-Z]\][ \t]+(.*)[ \t]+.*(:.*:)/\2/' | xargs)
	...
	if [[ -z "${seen[$title]}" ]] && [[ cnt -le 10 ]]; then
		...
		printf '  {"status": %s, "priority": %s, "title": "%s", "tags": "%s", "deadline": %s}' \
			   "$(map_status $status)" "$(map_priority $priority)" "$title" "$tags" "$(map_deadline "$deadline")"
		seen[$title]=1
		cnt=$((cnt+1))
	fi
